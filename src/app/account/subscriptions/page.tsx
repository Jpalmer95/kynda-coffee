...
export default async function SubscriptionsPage() {
  ...
  const hasActive = subscriptions?.some((s: any) => s.status === "active");
        ...
        {(!subscriptions || subscriptions.length === 0) ? (
          ...
          <div className="mt-6 space-y-4">
            {subscriptions.map((sub: any) => {
              const productName = sub.products?.name ?? "Coffee Club";
              const priceCents = sub.products?.price_cents ?? 0;
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="text-left sm:text-right">
                      <p className="font-heading text-lg font-bold text-espresso">
                        {formatPrice(priceCents)}/{sub.frequency === "weekly" ? "wk" : sub.frequency === "biweekly" ? "2wk" : "mo"}
                      </p>
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-3 border-t border-latte/10 pt-4 sm:grid-cols-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-mocha">
                        Next: {sub.next_delivery_at
                          ? new Date(sub.next_delivery_at).toLocaleDateString()
                          : "TBD"}
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
